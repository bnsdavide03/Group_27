			...
			for (int j = 0; j < 5; j++) {
				...
				if (t1 != null) {
					...
					if (i < 3||j<2) //verificare utilità
					{
						if(i<3)
						{
							if (virtualLibrary.getTile(new Position(i + 1, j)) != null&&virtualLibrary.getTile(new Position(i + 2, j)) != null&&virtualLibrary.getTile(new Position(i + 3, j)) != null) {
								if (t1.getColor() == virtualLibrary.getTile(new Position(i + 1, j)).getColor()&&t1.getColor() == virtualLibrary.getTile(new Position(i + 2, j)).getColor()&&t1.getColor() == virtualLibrary.getTile(new Position(i + 3, j)).getColor()) {
									count++;
						...
						if(j<2)
						{
							if (virtualLibrary.getTile(new Position(i, j + 1)) != null&&virtualLibrary.getTile(new Position(i, j + 2)) != null&&virtualLibrary.getTile(new Position(i, j + 3)) != null)
							{
								if (t1.getColor() == virtualLibrary.getTile(new Position(i, j + 1)).getColor()&&t1.getColor() == virtualLibrary.getTile(new Position(i, j + 2)).getColor()&&t1.getColor() == virtualLibrary.getTile(new Position(i, j + 3)).getColor()) {
									count++;
									//System.out.println("Count: "+count);
